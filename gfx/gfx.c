...
char GFX_printBuf[50];
void GFX_printf(const char *format, ...)
{
	va_list args;
	va_start(args, format);
	vsprintf(GFX_printBuf, format, args);
	GFX_printString(GFX_printBuf);
	va_end(args);
}
